...
function jdmj:next_action(key, action)
...
function jdmj:join(name, info, room_card, agent)
    if name ~= "jdmj" then
        error{code = error_code.ERROR_CHESS_NAME}
    end
...
function jdmj:hu(id, msg)
    ...
    local user = self:hu_action(info, hu_type, mul, baotou, last_deal, last_index, contract)
    local ci = {
...
function jdmj:android_out(info)
        ...
        for k, v in ipairs(role) do
            if not v.pass then
                ...
                if op and op.hu_action and not self:check_hu_prior(self._gang_index, k) then
                    ...
                    local user = self:hu_action(v, hu_type, mul, baotou, last_deal, last_index, contract)
                    local ci = {
                        status=self._status, count=self._count, banker=self._banker,
                    }
                    broadcast(user, ci, role, id)
