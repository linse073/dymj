...
local function valid_card(c)
    return c>0 and c<=base.POKER_CARD_FULL
end
...
function jhbj6:init(number, rule, rand, server, card)
    self._number = number
    self._rule = rule
    self._rand = rand
    self._server = server
...
function jhbj6:status(id, status, addr)
...
function jhbj6:custom_card(name, card)
    if name ~= "jhbj6" then
...
function jhbj6:pack(id, ip, agent, location)
...
function jhbj6:enter(info, agent, index, location)
    local role = self._role
...
function jhbj6:join(info, room_card, agent, location)
...
function jhbj6:chat_info(id, msg)
    local info = self._id[id]
    if not info then
        error{code = error_code.NOT_IN_CHESS}
    end
...
function jhbj6:location_info(id, msg)
    local info = self._id[id]
    if not info then
        error{code = error_code.NOT_IN_CHESS}
    end
...
function jhbj6:op_check(id, status)
...
local function special(extra, card, type_card)
    local color = {}
    local value = {}
    local jokers = {} --大小王替换值,大小王不能当炸弹 
    local ccount, vcount = 0, 0
    for k, v in ipairs(card) do
        local cl, vl = func.poker_info(v)
        if v > base.PBJ_WILDCARD_RATE then --大小王
            local wildcard = jokers[vl]
            if not wildcard then
                jokers[vl] = {}
                wildcard = jokers[vl]
            end
            wildcard[#wildcard+1] = v
        end
        local ci = color[cl]
        if ci then
            ci[#ci+1] = vl
    ...
    for k, v in ipairs(type_card) do
        if v.pt == base.PBJ_TYPE_SANTIAO then
            ...
            if value[v.comp[1].v] >= 4 then
                -- 大小王不能当炸弹
                local wildcard = jokers[v.comp[1].v]
                if wildcard then
                    if (value[v.comp[1].v] - #wildcard)>=4 then
                        count4 = count4 + 1
                    end
                else
                    count4 = count4 + 1
...
function jhbj6:bj_out(id, msg)
    ...
    for k,v in ipairs(temp_card) do
        temp_card[k] = func.restitute_poker_value(v)
    end
    ...
    local _card = util.clone(card)
    table.sort(_card, func.sort_poker_value)
    info.out_index = special(self._rule.extra, _card, type_card)
